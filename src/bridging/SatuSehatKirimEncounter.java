    ...
    private void tampil() {
        ...
        try{
            ps=koneksi.prepareStatement(
                   ...
                   "reg_periksa.status_lanjut,date_format(mutasi_berkas.kembali,'%Y-%m-%dT%H:%i:%s+08:00') as pulang,ifnull(satu_sehat_encounter.id_encounter,'') as id_encounter "+
                   ...
                   "inner join mutasi_berkas on mutasi_berkas.no_rawat=reg_periksa.no_rawat left join satu_sehat_encounter on satu_sehat_encounter.no_rawat=reg_periksa.no_rawat "+
            ...
            ps=koneksi.prepareStatement(
                   "select reg_periksa.tgl_registrasi,reg_periksa.jam_reg,reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.no_ktp,reg_periksa.kd_dokter,"+
                   "pegawai.nama,pegawai.no_ktp as ktpdokter,reg_periksa.kd_poli,poliklinik.nm_poli,satu_sehat_mapping_lokasi_ralan.id_lokasi_satusehat,reg_periksa.stts,"+
                   "reg_periksa.status_lanjut,concat(kamar_inap.tgl_keluar,'T',kamar_inap.jam_keluar,'+08:00') as pulang,ifnull(satu_sehat_encounter.id_encounter,'') as id_encounter "+
                   "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis inner join pegawai on pegawai.nik=reg_periksa.kd_dokter "+
                   "inner join poliklinik on reg_periksa.kd_poli=poliklinik.kd_poli inner join satu_sehat_mapping_lokasi_ralan on satu_sehat_mapping_lokasi_ralan.kd_poli=poliklinik.kd_poli "+
                   "inner join kamar_inap on kamar_inap.no_rawat=reg_periksa.no_rawat left join satu_sehat_encounter on satu_sehat_encounter.no_rawat=reg_periksa.no_rawat "+
