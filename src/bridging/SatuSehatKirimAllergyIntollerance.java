...
public final class SatuSehatKirimAllergyIntollerance extends javax.swing.JDialog {
    ...
    public SatuSehatKirimAllergyIntollerance(java.awt.Frame parent, boolean modal) {
        ...
        int[] columnWidths = {20, 150, 105, 70, 150, 110, 80, 150, 110, 80, 140, 210, 63, 63, 150, 215, 150, 150, 150, 150, 150};

        for (int i = 0; i < columnWidths.length; i++) {
            tbKamar.getColumnModel().getColumn(i).setPreferredWidth(columnWidths[i]);
        }
    ...
    private void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "SELECT reg_periksa.no_rawat, pasien.nm_pasien, pasien.no_ktp, pegawai.nama, pegawai.no_ktp AS ktpdokter, "
                    + "poliklinik.nm_poli, satu_sehat_mapping_lokasi_ralan.id_lokasi_satusehat, reg_periksa.stts, reg_periksa.status_lanjut, "
                    + "reg_periksa.kd_dokter, reg_periksa.kd_poli, reg_periksa.no_rkm_medis, satu_sehat_encounter.id_encounter, "
                    + "alergi_pasien.allergy_code, satu_sehat_ref_allergy.system, satu_sehat_ref_allergy.display, alergi_pasien.note, "
                    + "DATE_FORMAT(alergi_pasien.tgl_perawatan, '%Y-%m-%dT%H:%i:%s+08:00') AS tgl_perawatan, alergi_pasien.nippetugas, "
                    + "petugas.nama AS petugasallergy, alergi_pasien.category, satu_sehat_allergy.id_allergy FROM reg_periksa "
                    + "INNER JOIN pasien ON reg_periksa.no_rkm_medis = pasien.no_rkm_medis INNER JOIN pegawai ON pegawai.nik = reg_periksa.kd_dokter "
                    ...
                    + "LEFT JOIN satu_sehat_allergy ON reg_periksa.no_rawat = satu_sehat_allergy.no_rawat INNER JOIN pegawai AS petugas ON petugas.nik = alergi_pasien.nippetugas "
                    + "WHERE " + belumterkirim + " reg_periksa.tgl_registrasi BETWEEN ? AND ? "
                    + (TCari.getText().equals("") ? "" : "AND (reg_periksa.no_rawat LIKE ? OR reg_periksa.no_rkm_medis LIKE ? OR pasien.nm_pasien LIKE ? OR pasien.no_ktp LIKE ? OR pegawai.nama LIKE ? OR poliklinik.nm_poli LIKE ? OR reg_periksa.stts LIKE ? OR reg_periksa.status_lanjut LIKE ?)")
