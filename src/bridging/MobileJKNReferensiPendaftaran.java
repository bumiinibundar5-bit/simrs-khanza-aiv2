...
public final class MobileJKNReferensiPendaftaran extends javax.swing.JDialog {
        ...
        if(tbJnsPerawatan.getSelectedRow()!= -1){
            try{
                ps=koneksi.prepareStatement(
                    ...
                    "WHERE referensi_mobilejkn_bpjs.statuskirim='Belum' and referensi_mobilejkn_bpjs.nobooking='"+tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),14).toString()+"'"+
                    ...
                    while(rs.next()){
                        try {
                            ...
                            requestJson ="{" +
                                            ...
                                            "\"keterangan\": \"Ingin Merubah Jadwal di Hari Lain\"" +
                                        "}";
                            ...
                            if(nameNode.path("code").asText().equals("200")||nameNode.path("code").asText().equals("208")){
                                Sequel.mengedittf("referensi_mobilejkn_bpjs","nobooking=?","status='Batal',validasi=now()",1,new String[]{
    ...
    private void BtnCheckinActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCheckinActionPerformed
        if(tbJnsPerawatan.getSelectedRow()!= -1){
                    ...
                    while(rs.next()){
                        try {
                            ...
                            if(nameNode.path("code").asText().equals("200")||nameNode.path("code").asText().equals("208")){
                                ...
                                Sequel.mengedittf("referensi_mobilejkn_bpjs","nobooking=?","status='Checkin',validasi=now(),statuskirim='Sudah'",1,new String[]{
                                    tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),14).toString()});
                            }
                            System.out.println("respon WS BPJS Kirim Referensi JKN : "+nameNode.path("code").asText()+" "+nameNode.path("message").asText()+"\n");
                        }catch (Exception ex) {
            ...
            JOptionPane.showMessageDialog(null,"Silahkan pilih dulu data yang mau dicekinkan..!!");
        }
